...
class Calendar extends CI_Controller {
	...
	public function index() 
	{
		...
		{
			$calendar[] = array(
							'reservation_ID' 	=> intval($val->reservation_ID), 
							'title' => $val->dropoff_Location, 
							'description' => trim($val->pickup_Location), 
							'start' => date_format( date_create($val->pickup_Date) ,"Y-m-d H:i:s"),
							'end' 	=> date_format( date_create($val->dropoff_Date) ,"Y-m-d H:i:s"),
							'color' => $val->color,
							);
		}
		...
		$this->load->view('include/footer_calendar');    
	}
	    ...
	    if ($this->form_validation->run() == TRUE)
      	{
			...
			if($calendar_id == 0)
			{
		        ...
		        if ($insert > 0) 
		        {
		    		...
		    		$response['reservation_ID']		= $insert;
		        }
		        else
		        {
			...
			{	
				$where 		= [ 'reservation_ID'  => $calendar_id];
	            ...
	            if ($update > 0) 
	            {
	            	$response['status'] = TRUE;
		    		$response['notif']	= 'Success add calendar';
		    		$response['reservation_ID']		= $calendar_id;
	            }
	            else
		        {
		        	$response['status'] = FALSE;
	...
	{
		$response 		= array();
		$calendar_id 	= $this->input->post('reservation_ID');
